...
$stmt = $conn->prepare("SELECT tbl_has_file.path,tbl_has_file.name_of_file 
FROM tbl_quotation JOIN tbl_has_file ON tbl_quotation.id = tbl_has_file.quotation;
WHERE 1");
$stmt->execute();

$row = $stmt->fetch(PDO::FETCH_ASSOC) ;
$quotation_file_path = $row['path'];
$quotation_file_name = $row['name_of_file'];
...
<body id="page-top">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped" id="dataTable" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        ...
                                        data-target="#update" data-id="<?= $k['id']; ?>">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3 row">
                            <div for="inputname" class="col-sm-2 col-form-label">ใบเสนอราคา</div>
                            <div class="col-sm-10">


                                <a id="qpdf" class="btn btn-primary btn-sm">กำลังโหลด</a>
    ...
    <script>
        function openModal(button) {
            var id = button.getAttribute('data-id'); // Extract id from data-id attribute
            // document.querySelector('.btn-primary.btn-sm').href = "/quotation.php?compane=" + id;
            // Use AJAX to send the id to the server and get the results
            $.ajax({
                type: "POST",
                url: "ajax_read_file.php", // Replace with the actual path to your PHP script
                data: {
                    id: id
                },
                success: function (data) {
                    // Handle the returned data (e.g., update the modal content)
                    console.log(data);

                    // Parse the JSON data (assuming it's returned as JSON)
                    var result = JSON.parse(data);

                    // Update the href and text of the 'qpdf' link
                    var qpdfLink = document.getElementById('qpdf');
                    if (result && result.path && result.name_of_file) {
                        // If result is defined and contains necessary properties
                        qpdfLink.href = result.path;
                        qpdfLink.innerText = result.name_of_file;
                    } else {
                        // If result is undefined or doesn't contain necessary properties
                        qpdfLink.href = "/quotation.php?compane=" +
                            id; // Set a default href or handle as needed
                        qpdfLink.innerText = "สร้างใหม่";
                    }
